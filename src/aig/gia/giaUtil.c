...
int Gia_ObjRecognizeExor( Gia_Obj_t * pObj, Gia_Obj_t ** ppFan0, Gia_Obj_t ** ppFan1 )
{
    ...
    if ( Gia_ObjFanin0(p0) != Gia_ObjFanin0(p1) || Gia_ObjFanin1(p0) != Gia_ObjFanin1(p1) )
        return 0;
    // printf("%d, %d\n",Gia_ObjFaninC0(p0), Gia_ObjFaninC0(p1));
...
int Gia_NodeRef_rec( Gia_Man_t * p, Gia_Obj_t * pNode, int fMark )
{
    ...
    if ( Gia_ObjIsCi(pNode) )
        return 0;
    assert( Gia_ObjIsAnd(pNode) );
    if ( fMark ) Gia_ObjSetTravIdCurrent(p, pNode); // the order of pNode been traversed
    pFanin = Gia_ObjFanin0(pNode);
...
/**
    Synopsis    [Returns the size of the MFFC and collects it in vSupp.]

    Description []
               
    SideEffects [The MFFC is traversed and stored in vSupp.]

    SeeAlso     [Gia_NodeMffcSize Gia_NodeMffcSizeMark]
*/
